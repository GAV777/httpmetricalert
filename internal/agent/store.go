// Package agent предоставляет хранилище метрик для агента.
//
// Store — потокобезопасная структура для сбора и хранения метрик
// перед отправкой на сервер. Использует sync.RWMutex для защиты
// от гонок при одновременном чтении и записи.
package agent

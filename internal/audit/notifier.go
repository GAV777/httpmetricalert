// Package audit предоставляет систему аудита метрик через паттерн «Наблюдатель».
//
// Поддерживаются два типа наблюдателей:
//   - FileObserver — записывает JSON-события в файл
//   - HTTPObserver — отправляет POST-запросы с JSON на удалённый URL
//
// Notifier управляет списком наблюдателей и уведомляет их о событиях.
package audit
